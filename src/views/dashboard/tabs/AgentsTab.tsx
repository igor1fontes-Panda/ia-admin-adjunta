...
export function AgentsTab({ activity, memory }: { activity: Activity[]; memory: AgentMemoryRow[] }) {
  ...
  const memoryFor = (agentSlug: string): AgentMemoryRow[] => memory.filter((m) => m.agent === agentSlug);
  const activeAgents = agents.filter((agent) => agent.runs > 0 && !agent.stale).length;
  const runtimePlan = useMemo(
    () =>
      buildAgentPromptPlan({
        agent: "product_studio",
        sessionId: "dashboard-session",
        currentTask: "assemble evidence-driven product pack",
        leads: [],
        clients: [],
        orders: [],
        activity,
      ...
      {section === "agents" ? (
        <>
          <section className="card overflow-hidden border-gold-500/20 p-6" aria-labelledby="autonomy-heading">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold-400">
                      {t("dash.agents.protocol")}
                    </p>
                    <h2 id="autonomy-heading" className="mt-1 text-lg font-bold text-zinc-50">
                      {t("dash.agents.ready")}
                    </h2>
                  </div>
                </div>
            ...
            <div
              className="mt-5 rounded-xl border border-sky-500/20 bg-sky-500/5 p-4"
              aria-label="Prompt runtime status"
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-xs font-semibold uppercase tracking-wider text-sky-300">
                  {t("dash.agents.promptTitle")}
                </p>
                <span className="badge bg-sky-500/10 text-sky-300">Static prefix v1</span>
              </div>
              ...
              <div className="mt-3 grid gap-2 text-[11px] text-zinc-500 sm:grid-cols-3">
                <span>
                  {t("dash.agents.tools")} {runtimePlan.tools.length}
                </span>
                <span>
                  {t("dash.agents.cacheKey")} {runtimePlan.cacheKey}
                </span>
                <span>
                  {t("dash.agents.bytes")} {runtimePlan.cacheableBytes}
                </span>
              </div>
            </div>
          </section>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    >
                      {a.runs > 0
                        ? stale
                          ? t("dash.agents.idle")
                          : t("dash.agents.active")
                        : t("dash.agents.waiting")}
                    </span>
                  </div>
                  ...
                  {memoryFor(slug(a.name)).map((m) => (
                    <div key={m.key} className="mt-3 rounded-xl border border-gold-500/20 bg-gold-500/5 px-3 py-2">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-gold-400">
                        {t("dash.agents.learned")} {m.key.replace(/_/g, " ")}
                      </p>
...
export function AcademyView({ academy }: { academy: ReturnType<typeof academyState> }) {
  const t = useT();
  const gradLabel = (g: AcademyStudent["graduation"]): string => t(`academy.graduation.${g}`);
  return (
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <p className="text-sm font-semibold text-zinc-200">
              {academy.teacherLastRun ? timeAgo(academy.teacherLastRun) : t("academy.noClass")}
            </p>
          </div>
        </div>
      ...
      <section>
        <div className="flex items-end justify-between gap-3">
          ...
          <span className="badge bg-emerald-500/15 text-emerald-300">
            {academy.trainedCount}/{academy.students.length} {t("academy.graduation.trained").toLowerCase()}
          </span>
        </div>
                ...
                {s.skills.map((sk) => (
                  <span
                    ...
                    className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                      sk.learned ? "bg-gold-500/15 text-gold-300" : "border border-white/10 text-zinc-500"
                    }`}
                  >
                    {sk.learned ? "✓ " : "○ "}
                    {sk.id.replace(/_/g, " ")}
              ...
              <div className="mt-4 rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  {s.marketBrief ? (
                    ...
                    >
                      {t(`academy.freshness.${briefFreshness(s.marketBrief.updatedAt)}`)} ·{" "}
                      {timeAgo(s.marketBrief.updatedAt)}
                    </span>
                  ) : null}
                </div>
                  ...
                  <div className="mt-2 space-y-1.5 text-xs leading-relaxed">
                    {typeof (s.marketBrief.value as Record<string, unknown>).market_state === "string" ? (
                      <p className="text-zinc-300">
                        <span className="font-semibold text-cyan-300">{t("academy.marketState")}: </span>
                        {String((s.marketBrief.value as Record<string, unknown>).market_state)}
                      </p>
                    ) : null}
                    {typeof (s.marketBrief.value as Record<string, unknown>).instruction === "string" ? (
                      <p className="text-zinc-300">
                        <span className="font-semibold text-cyan-300">{t("academy.instruction")}: </span>
                        {String((s.marketBrief.value as Record<string, unknown>).instruction)}
                      </p>
                    ) : null}
                    {typeof (s.marketBrief.value as Record<string, unknown>).avoid === "string" ? (
                      <p className="text-zinc-500">
                        <span className="font-semibold">{t("academy.avoid")}: </span>
                        {String((s.marketBrief.value as Record<string, unknown>).avoid)}
                      </p>
                    ) : null}
                  </div>
                ) : (
